	  		...
	  		<table class="table table-bordered">
  				...
  				<?php
					...
					while ($row = mysqli_fetch_assoc($rs)){?>
		  			...
		  			<tr>
						<td><?=$row['order_id'] ?></td>
						<td><?=($row['customer_id'] == -1) ? 'Guest' :getCustomerNameFromId($con, $row['customer_id']) ?></td>
						<td><?=getOrderItemQty($con, $row['order_id']) ?></td>

						<td><?=$row['grand_total'] ?></td>
						<td><a href="../<?=$row['prescription'] ?>">prescription copy</a> </td>

						<td><?=$row['payment_status'] ? 'Payed' : 'Not Payed' ?></td>
						<?php 
						$status = null;
